...
Ext.onReady(function() {
	...
	// 订单列表存储块
	applyOrdersStore = Ext.create("Ext.data.Store", {
		autoLoad : true,
		fields : ["ordrNum", "industryTestName", "rent", "applyDate", "status",
				"desc", "subBeginDate", "subEndDate", "id", "principal",
				"phone", "sysDesc","url"],
		...
		proxy : {
			...
			url : "applyOrderManager/applyOrder!queryIndustryOrderByUser.action",
			reader : {
				type : "json",
				root : "cloudContext.params.industryTestOrders",
	...
	var applyOrdersGrid = new Ext.grid.Panel({
		...
		plugins : [{
			...
			rowBodyTpl : new Ext.XTemplate("<div class='detail_wrapper'>",
					...
					"<div class='detail_item'>负责人: {principal}</div>",
		...
		columns : [Ext.create("Ext.grid.RowNumberer"), {
					...
					tpl : '<tpl if="status == 1 || status == 2 || status == 0 || status == 3 ||  status == 4 || status == 5 || status == 6">'
							+ '<a style="text-decoration:none;margin-right:5px;" href="javascript:closeApplyOrderOrder({id});">'
							+ '<img src="extjs/resources/icons/closeworkorder.png"  title="关闭订单" alt="关闭订单" class="actionColumnImg" />&nbsp;</a>'
							+ '</tpl>'
							+ '<tpl if="url!=null">'
							+ '<a style="text-decoration:none;" href="javascript:downloadReport({id});">'
							+ '<img src="extjs/resources/icons/arrow_down.png" alt="下载检测报告" title="下载用户检测报告"  class="actionColumnImg" /></a>&nbsp;'
							+ '</tpl>'
		...
		dockedItems : [{
			...
			items : [{
						...
						handler : function() {
							Ext.getCmp('searchOrderNumId').setValue('');
							// 刷新列表
							query();
						}
					}]
...
function closeApplyOrderOrder(id) {
	Ext.MessageBox.prompt("是否关闭订单", "请输入理由", function(e, value) {
		if (e == "ok") {
			CKGobal.ajax({
...
function downloadReport(id) {
	 var basePath="applyOrderManager/applyOrder!download.action";
	 window.location = basePath + "?cloudContext.vo.id="+id;
}
